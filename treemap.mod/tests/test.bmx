...
Type TTreeMapTest Extends TTest
    ...
    Method TestRemoveMissingAndLeafOneChildTwoChildren() { test }
        ...
        For Local n:IMapNode<String,String> = EachIn m
            If prev <> Null Then AssertTrue(prev <= n.GetKey(), "In-order after removals")
            prev = n.GetKey()
        Next
    End Method
    ...
    Method TestIterationOrderSorted() { test }
        ...
        For Local n:IMapNode<String,String> = EachIn m
            If prev <> Null Then
                AssertTrue(prev <= n.GetKey(), "Keys are yielded in non-decreasing order")
            End If
            prev = n.GetKey()
        Next
    ...
    Method TestCustomComparatorReverseOrder() { test }
        ...
        Local i:Int = 0
        For Local n:IMapNode<String,Int> = EachIn m
            AssertEquals(expect[i], n.GetKey(), "Custom comparator iteration order")
            i :+ 1
        Next
    End Method
    ...
    Method TestFuzzInsertRemoveAndOrder() { test }
        ...
        Local prev:Int = 0
        For Local n:IMapNode<Int,Int> = EachIn m
            If Not first Then AssertTrue(prev <= n.GetKey(), "In-order after fuzz inserts")
            prev = n.GetKey()
            first = False
        Next
        ...
        For Local n2:IMapNode<Int,Int> = EachIn m
            AssertTrue(prev2 <= n2.GetKey(), "In-order after fuzz removals")
            prev2 = n2.GetKey()
        Next
    End Method
