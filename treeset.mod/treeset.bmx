...
Type TTreeSet<T> Implements ISet<T>
	...
	Rem
	bbdoc: Creates a new #TTreeSet instance using the default comparator.
	End Rem
	Method New()
	End Method
	
	Rem
	bbdoc: Creates a new #TTreeSet instance using the specified comparator.
	End Rem
	...
	Rem
	bbdoc: Creates a new #TTreeSet initialised by @array.
	End Rem
	...
	Rem
	bbdoc: Creates a new #TTreeSet initialised by @iterable.
	End Rem
	...
	Rem
	bbdoc: Returns an iterator that iterates through the #TTreeSet.
	End Rem
	Method GetIterator:IIterator<T>()
		Return New TTreeSetIterator<T>(FirstNode())
	End Method
	...
	Rem
	bbdoc: Gets the number of elements contained in the #TTreeSet.
	End Rem
	...
	Rem
	bbdoc: Returns #True if the #TTreeSet is empty, otherwise #False.
	End Rem
	...
	Rem
	bbdoc: Adds an element to the #TTreeSet and returns a value that indicates if it was successfully added.
	End Rem
	Method Add:Int(element:T)
		Local node:TTreeSetNode<T>=root
		Local parent:TTreeSetNode<T>
		Local cmp:Int
	...
	Rem
	bbdoc: Removes any element in the current #TTreeSet that is also in @other.
	End Rem
	Method Complement(other:IIterable<T>) Override
		If Not other Then
		...
		Local set:TTreeSet<T> = TTreeSet<T>(other)
		If set Then
	...
	Rem
	bbdoc: Determines whether the #TTreeSet contains the specified key.
	returns: #True if the #TTreeSet contains an element with the specified key; otherwise, #False.
	End Rem
	...
	Rem
	bbdoc: Returns a view of a subset in the #TTreeSet.
	...
	This method does not copy elements from the #TTreeSet, but provides a window into the underlying #TTreeSet itself.
	You can make changes in both the view and in the underlying #TTreeSet.
	End Rem
	Method ViewBetween:TTreeSet<T>(lowerValue:T, upperValue:T)
	...
	Rem
	bbdoc: Modifies the current #TTreeSet so that it contains only elements that are also in a specified #IIterable.
	End Rem
	Method Intersection(other:IIterable<T>) Override
		If Not other Then
			Throw New TArgumentNullException("other")
		End If
	...
	Rem
	bbdoc: Determines whether the #TTreeSet object is a proper subset of the specified #IIterable.
	returns: #True if the #TTreeSet is a proper subset of @other; otherwise, #False.
	...
	by the current #TTreeSet is empty unless @other is also an empty set.

	This method always returns #False if #Count is greater than or equal to the number of elements in @other.
	End Rem
	Method IsProperSubsetOf:Int(other:IIterable<T>) Override
		Return DoIsSubsetOf(other, True)
	End Method
	
	Rem
	bbdoc: Determines whether the #TTreeSet is a proper superset of the specified #IIterable.
	returns: #True if the #TTreeSet is a proper superset of @other; otherwise, #False.
	about: An empty set is a proper superset of any other collection. Therefore, this method returns #True if the
	collection represented by @other is empty unless the current #TTreeSet is also empty.

	This method always returns #False if #Count is less than or equal to the number of elements in @other.
	End Rem
	Method IsProperSupersetOf:Int(other:IIterable<T>) Override
		Return DoIsSupersetOf(other, True)
	End Method
	
	Rem
	bbdoc: Determines whether the #TTreeSet is a subset of the specified #IIterable.
	returns: #True if the current #TTreeSet is a subset of @other; otherwise, #False.
	about: An empty set is a subset of any other collection, including an empty set; therefore, this method returns
	#True if the collection represented by the current #TTreeSet is empty, even if the @other parameter is an empty set.

	This method always returns #False if #Count is greater than the number of elements in @other.
	End Rem
	Method IsSubsetOf:Int(other:IIterable<T>) Override
		Return DoIsSubsetOf(other, False)
	End Method
	
	Rem
	bbdoc: Determines whether the #TTreeSet is a superset of the specified #IIterable.
	returns: #True if the #TTreeSet is a superset of @other; otherwise, #False.
	about: All collections, including the empty set, are supersets of the empty set. Therefore, this method returns #True if
	the collection represented by @other is empty, even if the current #TTreeSet is empty.

	This method always returns #False if #Count is less than the number of elements in @other.
	End Rem
	Method IsSupersetOf:Int(other:IIterable<T>) Override
		Return DoIsSupersetOf(other, False)
	End Method
	
	Rem
	bbdoc: Determines whether the current #TTreeSet and a specified #IIterable share common elements.
	returns: #True if the #TTreeSet and @other share at least one common element; otherwise, #False.
	End Rem
	Method Overlaps:Int(other:IIterable<T>) Override
		If Not other Then
			Throw New TArgumentNullException("other")
		End If
	...
	Rem
	bbdoc: Removes a specified element from the #TTreeSet.
	returns: #True if the element is successfully found and removed; otherwise, #False. This method returns #False if the element is not found in the #TTreeSet.
	End Rem
	Method Remove:Int(element:T) Override
		Local node:TTreeSetNode<T> = FindNode(element)
		If node=Null Then
			Return False
		End If
		RemoveNode node
	...
	Rem
	bbdoc: Modifies the current #TTreeSet so that it contains only elements that are present either in the current set or in the specified #IIterable, but not both.
	End Rem
	Method SymmetricDifference(other:IIterable<T>) Override
		If Not other Then
			Throw New TArgumentNullException("other")
		End If
		...
		If Not set Then
			set = New TTreeSet<T>(other, comparator)
		End If
	...
	Method TryGetValue:Int(value:T, actualValue:T Var)
		Local node:TTreeSetNode<T> = FindNode(value)
		If node <> Null Then
	...
	Rem
	bbdoc: Modifies the current #TTreeSet so that it contains all elements that are present in either the current #TTreeSet or the specified #IIterable.
	End Rem
	Method UnionOf(other:IIterable<T>) Override
		If Not other Then
			Throw New TArgumentNullException("other")
		End If
		...
		If set And Not TSubSet<T>(Self) And Not size Then
			Local tmp:TTreeSet<T> = New TTreeSet<T>(other, comparator)
			root = tmp.root
			size = tmp.size
			version :+ 1
			Return
	...
	Rem
	bbdoc: Converts a #TTreeSet to an array.
	...
	Rem
	bbdoc: Creates a new #TTreeSet from an array.
	returns: A new #TTreeSet containing the elements from @array.
	End Rem
	Function FromArray:TTreeSet<T>(array:T[], comparator:IComparator<T> = Null)
...
Private
	...
	Method RotateRight( node:TTreeSetNode<T> )
		Local child:TTreeSetNode<T>=node.leftNode
	...
	Method RepairAdd( node:TTreeSetNode<T> )
		While node.parent And node.parent.colour=0 And node.parent.parent<>Null
			If node.parent = node.parent.parent.leftNode Then
				...
				EndIf
			Else
				Local uncle:TTreeSetNode<T>=node.parent.parent.leftNode
	...
	Method FindNode:TTreeSetNode<T>( element:T )
		Local node:TTreeSetNode<T>=root
		While node<>Null
	...
	Method FirstNode:TTreeSetNode<T>()
		Local node:TTreeSetNode<T> = root
	...
	Method LastNode:TTreeSetNode<T>()
		Local node:TTreeSetNode<T> = root
	...
	Method RemoveNode( node:TTreeSetNode<T> )
		Local splice:TTreeSetNode<T>
		Local child:TTreeSetNode<T>
		...
		Local parent:TTreeSetNode<T> = splice.parent
		If child <> Null Then
			child.parent = parent
		EndIf
		If parent = Null Then
	...
	Method RepairRemove(node:TTreeSetNode<T>, parent:TTreeSetNode<T>)

		Function IsBlack:Int(n:TTreeSetNode<T>) Inline
			' Null is black in RB-trees
			Return n = Null Or n.colour = 1
		End Function

		Function IsRed:Int(n:TTreeSetNode<T>) Inline
		...
		While node <> root And (node = Null Or node.colour = 1)
			...
			Local sibLeft:TTreeSetNode<T> = Null
			Local sibRight:TTreeSetNode<T> = Null
			If sib <> Null Then
	...
	Method FindRange:TTreeSetNode<T>(elementFrom:T, elementTo:T)
		Local currentNode:TTreeSetNode<T> = root
		While currentNode
	...
	Method DoIsSubsetOf:Int(other:IIterable<T>, isProper:Int)
		...
		Local otherSet:TTreeSet<T> = TTreeSet<T>(other)
		If otherSet Then
			If size > otherSet.size Then
				Return False
			End If

			Local prunedOther:TTreeSet<T> = otherSet.ViewBetween(FirstNode().element, LastNode().element)
	...
	Method DoIsSupersetOf:Int(other:IIterable<T>, proper:Int)
		...
		If set Then
			...
			Local pruned:TTreeSet<T> = ViewBetween(set.FirstNode().element, set.LastNode().element)
			For Local element:T = EachIn set
...
Public
	...
	Method NextNode:TTreeSetNode<T>()
		Local node:TTreeSetNode<T> = Self
		If node.rightNode<>Null
			...
			Return node
		EndIf
		Local parent:TTreeSetNode<T>=parent
	...
	Method PreviousNode:TTreeSetNode<T>()
		Local node:TTreeSetNode<T> = Self
		If node.leftNode<>Null
			...
			Return node
		EndIf
		Local parent:TTreeSetNode<T> = node.parent
...
Private
	...
	Method New(base:TTreeSet<T>, minimum:T, maximum:T)
		New(base.comparator)
		Self.base = base
